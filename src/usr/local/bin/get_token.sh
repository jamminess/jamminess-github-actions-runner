...
[[ -r "$GH_APP_PRIVATE_KEY_PATH" ]] || {
  echo "GitHub App private key is not readable: $GH_APP_PRIVATE_KEY_PATH" >&2
  exit 1
}

openssl pkey -in "$GH_APP_PRIVATE_KEY_PATH" -noout >/dev/null 2>&1 || {
  echo "GH_APP_PRIVATE_KEY_PATH does not contain a valid PEM private key" >&2
  exit 1
}
...
export GH_INSTALLATION_TOKEN GH_INSTALLATION_TOKEN_EXPIRES_AT
